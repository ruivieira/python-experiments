...
def element_to_tiddler(element) -> Tiddler:
    _tiddler = Tiddler(title=element['title'], fields=element.attrs, contents=[str(c) for c in element.find("pre").contents])
    ...
    if len(elems) > 0:
        for element in elems:
            if "title" in element.attrs and not element.attrs['title'].startswith("$:/"):
                ...
                OUTPUT_FILE = os.path.join(output, tiddler.title.replace("/", "-") + ".md")
                print(OUTPUT_FILE)
